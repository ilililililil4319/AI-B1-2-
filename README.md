...
이번 과제에서 사용한 AI 도구는 용도별로 다음과 같이 하나씩 명확하게 확정하였다(별도의 대체 도구는 사용하지 않음). 전체 작업은 **'기획 → 생성 → 검수 → 편집/통합'**의 4단계 파이프라인 순서로 설계하였다.


1단계, 기획(텍스트 기획) : Gemini / GPT / Claude — 브랜드 콘셉트·핵심 메시지·스토리보드 문서화 및 내레이션 초안 작성에 세 도구를 함께 활용하였다.
2단계, 생성(이미지→영상) : 먼저 Gemini로 장면별 키비주얼과 제품 히어로샷을 생성하였다. 이전 장면의 이미지를 참조 이미지로 함께 입력해 인물·스타일 일관성을 유지하고, 브랜드명(RAUM)·로고 같은 텍스트 요소를 이미지 안에 정확하게 렌더링할 수 있었다. 이후 Runway로 회원가입 시 받은 무료 크레딧을 활용해 AI 이미지를 AI 영상으로 변환하였다. 작업 순서는 [Custom] → 이미지 업로드 → 미리 준비한 프롬프트 입력 → 모델을 [Gen-4 Turbo]로 설정 → [Generate] 순으로 진행하였다.
3단계, 검수(QC) : Gemini 이미지 생성 직후, 그리고 Runway 영상 변환 직후 각각 결과물을 확인하는 검수 단계를 두었다. 구체적으로는 ① Gemini 이미지 3장을 나란히 비교해 인물·색감·브랜드 로고 표기가 이전 씬과 어긋나지 않는지 확인 → ② 기준에 맞지 않으면 프롬프트를 수정해 재생성(4장 프롬프트 수정 기록 참조) → ③ Runway 영상 변환 후에도 카메라 워크·모션이 의도한 톤과 맞는지 재확인 → ④ VideoStew에 최종 업로드하기 전, 세 씬의 해상도·화면비율·색감이 일치하는지 마지막으로 검수하는 4단계로 운영하였다(자세한 보정 기준은 15장 참조).
4단계, 편집/통합 : VideoStew — 검수를 통과한 이미지·영상 소스만 VideoStew에 업로드하여 편집하였다. 작업 순서는 [비주얼]→[사용자] 탭에서 Gemini·Runway로 생성해 둔 이미지/영상을 업로드 → 슬라이드에 추가 → 나레이션·자막·BGM·전환 편집 → 최종 영상 렌더링 후 다운로드(다운로드 시 크레딧 차감)하는 방식이다. CapCut은 사용하지 않는다. 나레이션은 AI 보이스 **'가현'**을 사용했고, 자막 폰트는 **'카페24 클래식타입'**을 적용했으며, 배경음악은 씬 구성에 맞춰 2개의 트랙을 슬라이드별로 나누어 사용하였다(Scene 1: NAVY CLOUD - feat. 사도, Scene 2~3: 큐디드라이 - EDM). 슬라이드 간 전환효과는 Auto로 설정하였다.
...
목표 메시지 : "바쁜 아침, 선이 늘 걸리는 불편함"을 3초 안에 직관적으로 각인시킨다.

화면 구성 : 화면은 모던한 화이트톤의 욕실에서 젖은 머리를 말리려는 인물이 유선 드라이기를 사용하는 모습으로 시작하는데, 짧고 팽팽하게 꼬인 검은색 코드가 벽면 콘센트에서 억지로 당겨지며 인물의 움직임을 제한하고, 그로 인한 불편함과 짜증이 표정과 자세에서 드러난다. 카메라 워크는 와이드 샷에서 시작해 서서히 줌인하며 손잡이에 어지럽게 감긴 전선과 인물의 초조한 표정을 담아내고, 마지막에는 세면대 모서리에 걸린 전선을 타이트하게 클로즈업하며 마무리한다.
...
출력 결과 요약 : 인물의 표정과 어지럽게 엉킨 전선이 3초 안에 직관적으로 각인되며, 다음 씬의 해결 장면과 명확히 대비되는 긴장된 톤을 확보하였다. 목표 메시지가 화면·나레이션·자막에 일관되게 반영되었음을 확인하였다.

Scene 2. 해결 제시 (개별 렌더링 4초)

목표 메시지 : RAUM의 무선성과 저소음 기능으로 Scene 1의 문제가 해결됨을 4초 안에 시각·청각적으로 보여준다.

화면 구성 : 화면은 동일한 욕실 배경에서, 씬1과 동일한 인물이 미소를 지으며 소형·경량화된 RAUM 무선 드라이기로 편안하게 머리를 말리는 모습을 담는다. 드라이기는 매트 화이트와 파스텔 라벤더 톤에 "RAUM" 텍스트와 로고가 선명하게 표시되어 있으며, 부드러운 바람의 흐름이 시각적으로 함께 표현된다. 카메라 워크는 미디엄 샷에서 시작해 서서히 돌리인하며, 편안하고 즐거운 표정으로 미소 짓는 인물이 자연스러운 손동작으로 머리를 매만지는 모습과 부드럽게 흩날리는 바람을 담아내고, 마지막에는 제품의 우아한 디자인을 강조하는 로우앵글 클로즈업으로 마무리한다.
...
출력 결과 요약 : 무선·저소음 핵심 기능이 시각·청각적으로 동시에 전달되며, 브랜드명·로고가 제품 표면에 자연스럽게 노출되어 씬1 대비 긴장→편안 톤 전환이 확인되었다. 목표 메시지(문제 해결)가 인물의 표정 변화와 나레이션으로 명확히 전달되었음을 확인하였다.

Scene 3. 브랜드 각인 (개별 렌더링 3.9초)

목표 메시지 : 제품의 핵심 혜택(무선·저소음)과 브랜드명(RAUM)을 마지막 3.9초 안에 각인시켜 행동을 유도한다.

화면 구성 : 화면은 제품을 충전 거치대에 올려놓는 히어로샷으로, 미니멀한 화이트와 라벤더 배경 위에 제품 표면에는 브랜드명(RAUM)과 로고가 이미 반영되어 있다. 카메라 워크는 제품 노즐 부분의 매크로 클로즈업에서 시작해, 살짝 위로 틸트되며 서서히 뒤로 빠지는 돌리 백으로 충전 거치대 위 제품 전체를 드러내고, 부드러운 조명이 매트한 표면을 우아하게 스치며, 마지막 0.5초는 완성도 높은 히어로샷으로 정지한다.
...
출력 결과 요약 : 제품의 핵심 혜택(무선·저소음)을 마지막으로 각인시키고, 브랜드명·로고는 제품 이미지 자체에 이미 반영되어 있어 나레이션·자막이 슬로건 대신 제품 혜택을 직접 전달하는 방식으로 마무리된다. 목표 메시지(브랜드 각인)가 화면·나레이션·자막에 일관되게 반영되었음을 확인하였다.
...
17. 다른 도구로 제작했을 때의 예상 차이 (품질/제어/속도/비용 관점)

동일한 스토리보드를 다른 조합의 AI 도구로 만들었다면 무엇이 가장 달라졌을지를 실제 사용 경험을 바탕으로 비교·예측하면 다음과 같다.

대체 시나리오품질제어속도비용예상되는 가장 큰 차이이미지 생성을 GPT로 대체준수하나 브랜드명·로고 텍스트 렌더링 정확도가 Gemini보다 다소 불안정참조 이미지 기반 일관성 유지는 가능하나 텍스트 요소 제어력은 Gemini가 더 우수유사유사씬마다 로고·브랜드명 표기가 미세하게 달라질 가능성이 높아, 검수·재생성 횟수가 늘어났을 것영상 생성을 미드저니(이미지) + 별도 애니메이션 도구로 대체이미지 자체의 예술적 완성도는 높으나 미드저니는 Image-to-Video를 직접 지원하지 않아 별도 변환 도구가 추가로 필요카메라 워크를 프롬프트로 세밀하게 제어하기 어려움도구가 하나 더 늘어 전체 파이프라인 속도 저하추가 도구 사용으로 비용·크레딧 소모 증가Runway 대비 카메라 무빙의 자연스러움과 작업 속도에서 손해를 보았을 것편집을 CapCut으로 대체컷 편집·자막 품질 자체는 우수나레이션(AI 보이스)과 자막을 한 도구 안에서 동시에 생성하지 못해 별도 TTS 도구 연동이 필요도구 연동 단계가 늘어 속도 저하무료 크레딧 기반 VideoStew 대비 유료 기능 의존도가 높아질 가능성VideoStew처럼 '생성+편집'을 한 도구에서 끝내는 효율성을 잃고, 도구 간 파일 이동(내보내기/불러오기) 과정에서 해상도·화질 손실 위험이 커졌을 것이미지/영상 생성을 구글 Flow, 구글 Omni 계열 통합 도구로 대체하나의 생태계 안에서 이미지→영상 전환 시 톤 일관성은 오히려 더 쉽게 유지될 가능성세부 카메라 워크·인물 표정 등 세밀한 프롬프트 제어는 Runway 대비 제한적일 수 있음이미지→영상 전환을 한 플랫폼에서 처리해 도구 전환 속도는 빨라질 가능성별도 크레딧 체계로 전체 비용 구조가 달라짐도구 전환 횟수는 줄지만, 카메라 워크의 세밀한 제어권은 다소 낮아졌을 것으로 예상

종합 예측 : 이미지·영상 생성 도구를 바꾸면 가장 먼저 영향을 받는 것은 **제어(카메라 워크·텍스트 렌더링 정확도)**이고, 편집 도구를 바꾸면 가장 먼저 영향을 받는 것은 **속도(도구 간 파일 이동 단계 증가)**이다. 현재 조합(Gemini→Runway→VideoStew)은 이미지 단계에서 비주얼을 100% 통제하고 영상 단계에서 모션만 추가하는 구조이므로, 다른 조합보다 재생성 횟수와 도구 전환 단계를 최소화할 수 있었다고 판단한다.


18. 영상 생성 크레딧 부족 시 대안 전략

만약 제작 도중 Runway의 영상 생성 크레딧이 예상보다 빨리 소진된다면, 다음 순서로 대응한다.


1순위, 씬 수 축소 : 3개 씬 중 재생성(재시도) 빈도가 가장 낮았던 Scene 2(해결 제시)의 개별 영상 렌더링은 그대로 유지하고, 상대적으로 프롬프트 재시도가 잦았던 Scene 1의 영상 변환 시도 횟수를 1회로 제한한다. 그래도 부족하면 Scene 1을 정지 이미지(Gemini 키비주얼)에 Ken Burns 효과(느린 줌인)만 적용한 정적 컷으로 대체하여, 영상 생성 자체를 건너뛴다.
2순위, 생성 횟수 제한 : 이미지(Gemini) 단계에서 비주얼을 충분히 검수·확정한 뒤에만 Runway 영상 변환을 시도해, 씬당 영상 생성 시도를 최대 2회(최초 1회 + 보정 1회)로 제한한다.
3순위, 대체 도구로 우회 : Runway 크레딧이 완전히 소진되면 Pika, Kling과 같은 대체 Image-to-Video 도구로 우회하거나, 구글 Flow·구글 Omni 같은 통합형 도구로 남은 씬만 처리한다. 이 경우 17장에서 예측한 대로 카메라 워크의 세밀한 제어력은 다소 낮아질 수 있음을 감안하여, 대체 도구 사용 씬은 카메라 워크 지시를 단순화한 프롬프트로 조정한다.
4순위, 편집 단계에서 보완 : 영상 소스 자체를 줄인 만큼, VideoStew 편집 단계에서 전환효과·자막 강조·BGM 다이내믹스를 더 적극적으로 활용해 축소된 영상 씬의 임팩트를 보완한다.
예방 조치 : 애초에 이미지 단계(Gemini)에서 비주얼을 완전히 확정한 뒤에만 영상 변환(Runway)을 시도하는 현재의 2단계 파이프라인 자체가, 크레딧이 가장 많이 소모되는 영상 생성 단계의 재시도 횟수를 최소화하기 위한 사전 예방 전략이었다.
